...
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION, ofp_version, htonl(0), 10);

    return buffer;
}

struct ofpbuf *
ofputil_encode_anomaly_detection_p2(enum ofputil_protocol protocol)
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION_O, ofp_version, htonl(0), 10);

    return buffer;
}

struct ofpbuf *
ofputil_encode_anomaly_detection2_p1(enum ofputil_protocol protocol)
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION_T_O, ofp_version, htonl(0), 1024);

    return buffer;
}

struct ofpbuf *
ofputil_encode_anomaly_detection2_p2(enum ofputil_protocol protocol)
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION_T_T, ofp_version, htonl(0), 1024);

    return buffer;
}

struct ofpbuf *
ofputil_encode_anomaly_detection3_p1(enum ofputil_protocol protocol)
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION_TH_O, ofp_version, htonl(0), 1024);

    return buffer;
}

struct ofpbuf *
ofputil_encode_anomaly_detection3_p2(enum ofputil_protocol protocol)
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION_TH_T, ofp_version, htonl(0), 1024);

    return buffer;
}

struct ofpbuf *
ofputil_encode_anomaly_detection4_p1(enum ofputil_protocol protocol)
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION_FO_O, ofp_version, htonl(0), 1024);

    return buffer;
}

struct ofpbuf *
ofputil_encode_anomaly_detection4_p2(enum ofputil_protocol protocol)
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION_FO_T, ofp_version, htonl(0), 1024);

    return buffer;
}

struct ofpbuf *
ofputil_encode_anomaly_detection4_p3(enum ofputil_protocol protocol)
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION_FO_TH, ofp_version, htonl(0), 1024);

    return buffer;
}

struct ofpbuf *
ofputil_encode_anomaly_detection4_p4(enum ofputil_protocol protocol)
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION_FO_F, ofp_version, htonl(0), 1024);

    return buffer;
}

struct ofpbuf *
ofputil_encode_anomaly_detection5_p1(enum ofputil_protocol protocol)
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION_FI_O, ofp_version, htonl(0), 1024);

    return buffer;
}

struct ofpbuf *
ofputil_encode_anomaly_detection5_p2(enum ofputil_protocol protocol)
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION_FI_T, ofp_version, htonl(0), 1024);

    return buffer;
}

struct ofpbuf *
ofputil_encode_anomaly_detection5_p3(enum ofputil_protocol protocol)
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION_FI_TH, ofp_version, htonl(0), 1024);

    return buffer;
}

struct ofpbuf *
ofputil_encode_anomaly_detection6_p1(enum ofputil_protocol protocol)
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION_SI_O, ofp_version, htonl(0), 1024);

    return buffer;
}

struct ofpbuf *
ofputil_encode_anomaly_detection6_p2(enum ofputil_protocol protocol)
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION_SI_T, ofp_version, htonl(0), 1024);

    return buffer;
}

struct ofpbuf *
ofputil_encode_anomaly_detection6_p3(enum ofputil_protocol protocol)
{
    enum ofp_version ofp_version = ofputil_protocol_to_ofp_version(protocol);
    struct ofpbuf *buffer = ofpraw_alloc_xid(OFPRAW_OFPT_ANOMALY_DETECTION_SI_TH, ofp_version, htonl(0), 1024);
